...
exports.getPaymentAccountById = async (req, res) => {
  try {
    const account = await PaymentAccount.findById(req.params.id).populate('activeStatusId');
    if (!account) {
...
exports.createPaymentAccount = async (req, res) => {
  try {
    const { accountName, activeStatusId } = req.body;
    if (!accountName) {
...
// Update a payment account (name or activeStatusId)
